...
import { Phone, MapPin, Plus, Route, ChevronDown, CheckCircle2, RefreshCw, Trash2 } from 'lucide-react'
import { StarRating, StatusBadge, StatusSelect, Modal } from '../ui'
import { CATEGORIES } from '../../lib/constants'
import { useAddCRMEntry, useDeleteClient } from '../../hooks/useClients'
...
export function ClientCard({ client, onSave, compact = false }) {
  ...
  const { addToRoute, routeClients, removeFromRoute } = useAppStore()
    ...
    <>
      <div className="card-hover p-4 animate-fade-in">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex flex-col gap-1 flex-shrink-0">
            {client.phone && (
              <a href={`tel:${client.phone}`} className="btn-ghost p-2 rounded-lg text-green-400 hover:text-green-300 hover:bg-green-500/10" title="Chiama">
                <Phone size={15} />
              </a>
            )}
            {onSave && (
              <button onClick={() => onSave(client)} className="btn-ghost p-2 rounded-lg text-brand-400 hover:text-brand-300 hover:bg-brand-500/10" title="Salva nel CRM">
                <Plus size={15} />
              </button>
            )}
            <button
            ...
            {/* Delete button — solo nel CRM (quando non c'è onSave) */}
            {!onSave && client.id && (
              <button
                onClick={(e) => { e.stopPropagation(); setShowDeleteConfirm(true) }}
                className="btn-ghost p-2 rounded-lg text-slate-600 hover:text-red-400 hover:bg-red-500/10"
      ...
      {showQuickEdit && (
        <div className="card border-t-0 rounded-t-none px-4 pb-4 -mt-1 animate-slide-up">
          ...
          />
          <button onClick={handleSaveStatus} disabled={addEntry.isPending} className="btn-primary mt-2 w-full justify-center text-xs">
      ...
      <Modal open={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} title="Elimina Cliente" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-slate-400">
            Sei sicuro di voler eliminare <span className="text-slate-100 font-semibold">{client.name}</span>?
            Verranno cancellati anche tutto lo storico CRM associato.
          </p>
          <div className="flex gap-3">
            <button onClick={() => setShowDeleteConfirm(false)} className="btn-secondary flex-1 justify-center">
              Annulla
            </button>
            <button onClick={handleDelete} disabled={deleteClient.isPending} className="btn-danger flex-1 justify-center">
              <Trash2 size={14} /> {deleteClient.isPending ? 'Eliminazione...' : 'Elimina'}
            </button>
          </div>
        </div>
      </Modal>
